...
uint8_t MCP342x::generalCallConversion(void)
{
  ...
  if(ioctl(fd_i2c, I2C_SLAVE, 0x00) < 0){
...
uint64_t micros(){
    ...
    uint64_t us = (((uint64_t)ts.tv_sec)*1000000) + (((uint64_t)ts.tv_nsec)/1000000);
    return us;
}
...
MCP342x::MCP342x(void) : address(0x69)
{
  ;
}
...
MCP342x::error_t MCP342x::convert(const Config &config) const
{
  ...
  buffer[0] = config.val | newConversionMask;
